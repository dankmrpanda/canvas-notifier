...
function getReminderEmbedColor(hoursLeft) {
  ...
  if (hoursLeft <= 12) return 0xffff00; // Yellow: < 12 hours
...
async function sendReminder(assignment, hoursLeft, channel, storedAssignments) {
  ...
  const reminders = { 24: "1 day left", 12: "12 hours left", 6: "6 hours left", 3: "3 hours left", 0.5: "30 minutes left" };
  const reminderKey = hoursLeft <= 0.5 ? 0.5 : hoursLeft <= 3 ? 3 : hoursLeft <= 6 ? 6 : hoursLeft <= 12 ? 12 : 24;
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Time Left', value: `<t:${Math.floor(new Date(assignment.deadline).getTime() / 1000)}:R>`, inline: true },
